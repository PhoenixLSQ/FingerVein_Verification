...
function [probe_calibrated, gallery_calibrated, row, col,corrmaxvalue] = Q_getCalibration_limit(probe, gallery)
    % 对 probe 和 gallery
...
% 图像大小为 72 x 156,所以讲范围限制到 row_min - row_max ,col_min - col_max
row_min = 58;
row_max = 86;
col_min = 125;
col_max = 187;
corrMap_lim = corrMap_selected(row_min:row_max,col_min:col_max);
    ...
    [row, col] = find(corrMap_selected == max(corrMap_lim(:)));%矩阵最大值对应的坐标即为平移量
